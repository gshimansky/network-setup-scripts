...
echo MACHINE 0
ssh -F $sc0 tctestvms-0 sudo -E ./network-setup-scripts/reconfigure.sh ./network-setup-scripts/lv/virtio/vlan-nat/client.sh
echo MACHINE 1
ssh -F $sc1 tctestvms-1 sudo -E ./network-setup-scripts/reconfigure.sh ./network-setup-scripts/lv/virtio/vlan-nat/middle.sh
echo MACHINE 2
ssh -F $sc2 tctestvms-2 sudo -E ./network-setup-scripts/reconfigure.sh ./network-setup-scripts/lv/virtio/vlan-nat/server.sh

$(cd /localdisk/tc_agent/vagrant; ./getports.sh 2375)
cd test/framework/main
./tf -directory nat-vlan-stabilityresults -config nat/stability-nat-vlan.json -hosts $NFF_GO_HOSTS
